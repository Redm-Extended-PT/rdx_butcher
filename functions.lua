...
NTRP.Spawn = function(name, x, y, z, heading)
...
NTRP.NPCText = function(text, s1, s2, x, y, r, g, b, font)
    SetTextScale(s1, s2)
...
Citizen.CreateThread(function()   
      if not DoesEntityExist(butcher01) then        
         butcher01 = NTRP.Spawn( Config.Butcher, Config.Butcher01_Pos, Config.Butcher01_Heading )
     end
      if not DoesEntityExist(butcher02) then
         butcher02 = NTRP.Spawn( Config.Butcher, Config.Butcher02_Pos , Config.Butcher02_Heading )        
      end
      if not DoesEntityExist(butcher03) then
         butcher03 = NTRP.Spawn( Config.Butcher, Config.Butcher03_Pos, Config.Butcher03_Heading )        
      end
      if not DoesEntityExist(butcher04) then
         butcher04 = NTRP.Spawn( Config.Butcher, Config.Butcher04_Pos, Config.Butcher04_Heading )      
      end
      if not DoesEntityExist(butcher05) then
         butcher05 = NTRP.Spawn( Config.Butcher, Config.Butcher05_Pos, Config.Butcher05_Heading )         
      end
      if not DoesEntityExist(butcher06) then
         butcher06 = NTRP.Spawn( Config.Butcher, Config.Butcher06_Pos, Config.Butcher06_Heading )      
      end      
end)
